...
// GPT-4.1 mini supports the structured JSON responses used by this pipeline and
// has a materially higher Tier 1 daily request allowance than GPT-5.4 mini.
const DEFAULT_MODEL = "gpt-4.1-mini";
// Keep enough throughput to clear the backlog while retaining a predictable
// per-run cost: one routing call plus one writing call per candidate.
const DEFAULT_LIMIT = 20;
const DEFAULT_MIN_REQUEST_INTERVAL_MS = 6_200;
const MAX_RATE_LIMIT_RETRIES = 4;
const ROUTER_PROMPT_CACHE_KEY = "ai-model-brief:router:v1";
const WRITER_PROMPT_CACHE_KEY = "ai-model-brief:writer:v1";
let nextRequestAt = 0;
...
function recencyScore(event) {
  const raw = event.publishedAt || event.updatedAt || event.date;
  if (!raw) return 0;
  const timestamp = Date.parse(raw);
  if (Number.isNaN(timestamp)) return 0;
  const ageDays = (Date.now() - timestamp) / 86_400_000;
  if (ageDays < 0) return 0;
  // Freshness must outweigh the severity of an old backlog item. Without a
  // strong recency bonus, high-severity historical deprecations can consume
  // every summarization slot while the current month's releases remain raw.
...
async function routeEventWithOpenAI(event, { apiKey, model }) {
  const parsed = await callOpenAIJson({
    apiKey,
    model,
    promptCacheKey: ROUTER_PROMPT_CACHE_KEY,
    maxOutputTokens: 200,
    input: [
      {
        role: "developer",
...
async function summarizeEvent(event, { apiKey, model, route }) {
  ...
  const parsed = await callOpenAIJson({
    apiKey,
    model,
    promptCacheKey: WRITER_PROMPT_CACHE_KEY,
    maxOutputTokens: 480,
    input: [
      {
        role: "developer",
...
async function callOpenAIJson({ apiKey, model, input, maxOutputTokens, promptCacheKey }) {
  for (let attempt = 0; attempt <= MAX_RATE_LIMIT_RETRIES; attempt += 1) {
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        ...
        prompt_cache_retention: process.env.OPENAI_PROMPT_CACHE_RETENTION || "24h"
      })
    });

    const payload = await response.json().catch(() => ({}));
    if (response.ok) return parseJsonObject(extractOutputText(payload));
...
async function waitForRequestSlot() {
  const interval = Number(process.env.OPENAI_MIN_REQUEST_INTERVAL_MS || DEFAULT_MIN_REQUEST_INTERVAL_MS);
  const now = Date.now();
  const waitMs = Math.max(0, nextRequestAt - now);
  if (waitMs) await sleep(waitMs);
  nextRequestAt = Date.now() + Math.max(0, interval);
}

function retryDelayMs(retryAfter, attempt) {
  const seconds = Number(retryAfter);
  if (Number.isFinite(seconds) && seconds > 0) return Math.ceil(seconds * 1_000) + 500;
  return Math.min(30_000, 6_500 * (attempt + 1));
}
