...
function eventFrom(source, title, body, url, ordinal = 0) {
  ...
  const normalizedBody = normalizeExtractedBody(body);
  const text = cleanText(`${normalizedTitle}\n${normalizedBody}`).slice(0, 12000);
  const modelIds = extractModelIds(text);
  const kind = classify(text, source.kind);
  return {
    id: hash(`${source.id}|${normalizedTitle}|${url}|${ordinal}`),
    ...
    summary: normalizedBody.slice(0, 1000),
    modelIds,
    sourceUrl: canonicalUrl(url, source.url),
    severity: severityFor(kind, text),
    publishedAt: source.id === "bedrock-lifecycle" ? null : parseDate(rawText) || parseDate(text),
    detectedAt: new Date().toISOString()
  };
}

function normalizeExtractedTitle(value = "") {
  return stripNewsChrome(cleanText(value));
}

function normalizeExtractedBody(value = "") {
  return stripNewsChrome(cleanText(value));
}
...
function parseDate(text) {
  const match = text.match(
    /\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]*,? \d{4}(?=\D|$)|\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Sept|Oct|Nov|Dec)[a-z]* \d{1,2},? \d{4}(?=\D|$)|\b\d{4}-\d{2}-\d{2}\b/
  );
...
function datedSections($, source) {
  ...
  $("h2,h3").each((index, element) => {
    ...
    const stopHeading = element.tagName === "h2" ? "h2" : "h2,h3";
    while (cursor.length && !$(cursor).is(stopHeading)) {
...
function headingSections($, source) {
  ...
  $("main h2, main h3, article h2, article h3, body h2, body h3").each(
    (index, element) => {
      ...
      if (/^(feedback|in this article|next steps|related content)$/i.test(title)) return;
      const chunks = [];
      let cursor = $(element).next();
...
function articleLinks($, source) {
  ...
  $("main a[href], article a[href], body a[href]").each((index, element) => {
    ...
    const metadataDate = linkPublishDate($(element));
    const context = cleanText(`${$(element).parent().text()}\n${metadataDate || ""}`);
    if (!parseDate(context) && !metadataDate && !/\/news\/|\/blog\//.test(href) && new URL(href).hostname !== "blog.google") return;
    seen.add(href);
    const event = eventFrom(source, title, context, href, index);
    event.publishedAt = parseLooseDate(metadataDate || "") || event.publishedAt;
    events.push(event);
  });
  return events.slice(0, 60);
}

function linkPublishDate(link) {
  const raw = link.attr("data-ga4-analytics-lead-click") || link.attr("data-analytics-lead-click") || "";
  const match = raw.match(/["']publish_date["']\s*:\s*["']([^"']+)["']/);
  return match?.[1] || "";
}
...
export function extractArticlePublishedAt(html) {
  ...
  const candidates = [
    ...
    $("time").first().text(),
    $("[class*='agate']").first().text()
  ].filter(Boolean);
  for (const candidate of candidates) {
    const parsed = parseLooseDate(cleanText(candidate));
    if (parsed) return parsed;
  }
  return null;
}

export function extractPublishedAtFromText(value) {
