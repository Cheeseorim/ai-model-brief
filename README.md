...
로컬에서 키를 붙여 테스트하려면 `.env.example`을 `.env`로 복사한 뒤 값을 채웁니다. 자연스러운 한국어 번역·요약을 사용하려면 저장소의 Actions secret에 `OPENAI_API_KEY`를 추가합니다. 기본 모델은 `gpt-4.1-mini`이며, 실행당 최대 요약 항목 수는 `SUMMARY_MAX_EVENTS`로 제한합니다.
